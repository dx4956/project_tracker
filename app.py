...
from flask import Flask, render_template, redirect, url_for, request, jsonify, flash, session, abort, Response, stream_with_context
...
class User(UserMixin, db.Model):
    ...
    memberships = db.relationship('ProjectMember', backref='user', lazy=True, foreign_keys='ProjectMember.user_id')
    ...
    def set_recovery_code(self):
        ...
        self.recovery_code_hash = generate_password_hash(raw)  # store raw (no dashes) as canonical
...
@login_required
def dashboard():
    owned = Project.query.filter_by(owner_id=current_user.id).order_by(Project.created_at.desc()).all()
...
def create_project():
    ...
    allowed_colors = ['indigo', 'violet', 'sky', 'emerald', 'rose', 'amber', 'pink', 'teal']
    ...
    project = Project(title=title, description=description, color=color, owner_id=current_user.id)
...
def project_board(project_id):
    ...
    members = [
        {'user': db.session.get(User, m.user_id), 'role': m.role}
...
def update_task(project_id, task_id):
    ...
    if request.method == 'DELETE':
        ...
        db.session.commit()
        _sse_push(project_id, 'task_deleted', {'id': task_id_copy, 'by': current_user.id})
...
def add_member(project_id):
    ...
    payload = {'id': user.id, 'username': user.username, 'initials': user.initials, 'email': user.email, 'by': current_user.id}
